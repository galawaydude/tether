 ...
 *
 * `RCAGENT_LOG_LEVEL` is there for the debugging session that wants the request
 * log; `TETHER_LOG_LEVEL` remains a compatibility alias. It is validated rather
 * than passed through: pino throws on a level it
...
function logLevel(): string {
  const wanted = process.env['RCAGENT_LOG_LEVEL'] ?? process.env['TETHER_LOG_LEVEL'];
  if (wanted === undefined || LOG_LEVELS.includes(wanted)) return wanted ?? 'warn';
  process.stderr.write(
    `rcagent: RCAGENT_LOG_LEVEL=${wanted} is not a level; using warn. ` +
